...
int main()
{
    //cout << "Your Age is : " << GetAgeInDays() << "Day(s)" << endl;
    //nFacRuntimeFunc(7);
    sDate Date1 = ReadDate();
    sDate Date2 = ReadDate();
    cout << "Difference is : "<< DifferenceBetweenDates2(Date1,Date2) << " Day(s)" << endl;
    if (DifferenceBetweenDates2(Date1,Date2) < 0)
        cout << "Difference (including end day) is :" << DifferenceBetweenDates2(Date1,Date2) - 1 << " Day(s)" << endl;
    else
        cout << "Difference (including end day) is :" << DifferenceBetweenDates2(Date1,Date2) + 1 << " Day(s)" << endl;
    /*Date1 = AddOneDayToDate(Date1);
    cout << "Date after adding one day is:" << Date1.Day << "/" << Date1.Month << "/" << Date1.Year << endl;
    if (IsDateLastDayInMonth(Date1))
        cout << "Yes day is last day in month\n";
    else
        cout << "No day is not last day in month\n";
    if (IsDateLastMonthInYear(Date1))
        cout << "Yes month is month in year\n";
    else
        cout << "No month is not last month in year\n";
    sDate Date2 = ReadDate();
    if (CheckDateEquailty1(Date1,Date2))
        cout << "Yes date 1 is equal to date 2\n";
    else
        cout << "No date 1 is not equal to date 2\n";*/
    if (CheckDateEquailty2(Date1,Date2))
        cout << "Yes date 1 is equal to date 2\n";
    else
        cout << "No date 1 is not equal to date 2\n";
    //if (CheckDates1(Date1,Date2))
    ...
    /*short day = ReadDay();
    sDate date  = GetNewDateAfterAddingDays(day);
    cout << "Date after adding ["<< day  <<"] days: " << date.Day << "/" << date.Month << "/" << date.Year;*/
    //main();
    return 0;
}
